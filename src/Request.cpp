...
void Request::parse(std::string request)
{
    ...
    std::string command =
        spaceFound != std::string::npos
            ? std::string(request.begin(), request.begin() + spaceFound)
            : request;

    if (std::find(commandList.begin(), commandList.end(), command) ==
        commandList.end())
    {
...
void Request::parseMailParameters(std::string rawParameter)
{
    ...
    if (startFound == 0 && startFound < endFound)
    {
        ...
        parameters.push_back(std::string(rawParameter.begin() + startFound + 6,
                                         rawParameter.begin() + endFound));
    }
    else
    {
...
void Request::parseRcptParameters(std::string rawParameter)
{
    ...
    if (startFound == 0 && startFound < endFound)
    {
        ...
        parameters.push_back(std::string(rawParameter.begin() + startFound + 4,
                                         rawParameter.begin() + endFound));
    }
    else
    {
